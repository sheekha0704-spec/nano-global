...
# --- PAGE CONFIG ---
st.set_page_config(page_title="NanoPredict AI v18.5 - Global Edition", layout="wide")
...
def load_and_prep(url):
    try:
        ...
        st.warning("Connecting to local backup database...")
    ...
    # Clean Categorical Data to prevent TypeError in sorted()
    ...
    for col in cat_cols:
        le = LabelEncoder()
        # Fit on ALL available data to prevent 'unseen labels' ValueError
        le.fit(df[col])
...
df, models, stab_model, le_dict = load_and_prep(DATABASE_URL)
...
# --- STEP 5: RESULTS ---
    ...
    res = {col: models[col].predict(input_df)[0] for col in models}
    ...
    with t3:
        ...
        for i, o in enumerate(o_rng):
            for j, s in enumerate(s_rng): z[i,j] = stab_model.predict(input_df)[0]
